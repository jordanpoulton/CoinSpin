...
contract CoinSpin {
	...
	uint constant HOUSE_EDGE_MIN = 0.0004 ether;
	uint constant MIN_BET = 0.01 ether;
	uint constant MAX_BET = 20 ether;

	uint constant MAX_OUTCOMES = 20;
	uint constant MAX_MASK_BITS = 40;
	uint constant MAX_BET_MASK = 2 ** MAX_MASK_BITS;

	uint constant BET_EXP_BLOCKS = 250;

	address constant DUMMY_ADDR = 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE;
	...
	uint128 public lockedBets;

	struct Bet {
		uint amount;
		uint8 betOptions;
		uint8 rollUnder;
		uint40 placeBetTxBlockNumber;
		uint40 mask;
    ...
    function setMaxBet(uint _maxBet) public onlyOwner {
        require (_maxBet < MAX_BET, "Size of bet should be a sane number.");
        maxProfit = _maxBet;
    }

    function withdrawFunds(address payable beneficiary, uint withdrawAmount) external onlyOwner {
        require (withdrawAmount <= address(this).balance, "Withdrawal amount larger than balance.");
        require (lockedBets + withdrawAmount <= address(this).balance, "Not enough funds (NB Locked Bets)");
        sendFunds(beneficiary, withdrawAmount, withdrawAmount);
    }

    function kill() external onlyOwner {
        require (lockedBets == 0, "All bets should be processed (settled or refunded) before self-destruct.");
        selfdestruct(owner);
    }

    function placeBet(uint betMask, uint betOptions, uint commitLastBlock, uint commit, bytes32 r, bytes32 s) external payable {
    	Bet storage bet = bets[commit];
        require (bet.bettor == address(0), "Bet should be in a 'clean' state.");

        uint amount = msg.value;
        require (betOptions > 1 && betOptions <= MAX_OUTCOMES, "Number of bettable scenarios should be within range.");
        require (amount >= MIN_BET && amount <= MAX_BET, "Stake should be within range.");
        ...
        if (betOptions <= MAX_MASK_BITS) {
        	rollUnder = ((betMask * POPCNT_MULT) & POPCNT_MASK) % POPCNT_MODULO;
            mask = betMask;
        } else {
            require (betMask > 0 && betMask <= betOptions, "Too many bet options.");
            rollUnder = betMask;
        }

        uint possibleWinAmount;

        possibleWinAmount = getSpinWinAmount(amount, betOptions, rollUnder);

        require (possibleWinAmount <= amount + maxProfit, "Maximum Profit Limit Exceeded");

        lockedBets += uint128(possibleWinAmount);

        require (lockedBets <= address(this).balance, "Contract cannot afford for you to win this bet");

        emit Commit(commit);

        bet.amount = amount;
        bet.betOptions = uint8(betOptions);
        bet.rollUnder = uint8(rollUnder);
        bet.placeBetTxBlockNumber = uint40(block.number);
    ...
    function settleBet(uint reveal, bytes32 blockHash) external onlyCroupier {
    	...
    	Bet storage bet = bets[commit];
    	uint placeBetTxBlockNumber = bet.placeBetTxBlockNumber;

    	require (block.number > placeBetTxBlockNumber, "settleBet in the same block as placeBet, or before.");
        require (block.number <= placeBetTxBlockNumber + BET_EXP_BLOCKS, "Blockhash can't be queried by EVM.");
        require (blockhash(placeBetTxBlockNumber) == blockHash);
    ...
    function settleBetUncleMerkleProof(uint reveal, uint40 canonicalBlockNumber) external onlyCroupier {
    	...
    	require (block.number <= canonicalBlockNumber + BET_EXP_BLOCKS, "Blockhash can't be queried by EVM.");
    ...
    function settleBetCommon(Bet storage bet, uint reveal, bytes32 entropyBlockHash) private {
    	...
    	uint betOptions = bet.betOptions;
    	uint rollUnder = bet.rollUnder;
    	address payable bettor = bet.bettor;
    	...
    	uint spin = uint(entropy) % betOptions;

    	uint spinWinAmount;
    	spinWinAmount = getSpinWinAmount(amount, betOptions, rollUnder);

    	uint spinWin = 0;

    	if (betOptions <= MAX_MASK_BITS) {
    		if ((2**spin) & bet.mask !=0) {
    			spinWin = spinWinAmount;
    		}
    	} else {
    		if (spin < rollUnder) {
    			spinWin = spinWinAmount;
    	...
    	lockedBets -= uint128(spinWinAmount);

    	sendFunds(bettor, spinWin == 0 ? 1 wei : spinWin, spinWin);
    ...
    function refundBet(uint commit) external {
    	...
    	uint amount = bet.amount;
    	require (amount != 0, "Bet should be in an 'active' state");
    	require (block.number > bet.placeBetTxBlockNumber + BET_EXP_BLOCKS, "Blockhash can't be queried by EVM");
    	bet.amount = 0;
    	uint spinWinAmount;
    	spinWinAmount = getSpinWinAmount(amount, bet.betOptions, bet.rollUnder);
    	lockedBets -= uint128(spinWinAmount);
    	sendFunds(bet.bettor, amount, amount);
    }

    function getSpinWinAmount(uint amount, uint betOptions, uint rollUnder) private pure returns (uint winAmount) {
    	require (0 < rollUnder && rollUnder <= betOptions, "Win probability out of range.");

        uint houseEdge = amount * HOUSE_EDGE_PERCENT / 100;

        if (houseEdge < HOUSE_EDGE_MIN) {
            houseEdge = HOUSE_EDGE_MIN;
        }

        require (houseEdge <= amount, "Bet doesn't even cover house edge.");
        winAmount = (amount - houseEdge) * betOptions / rollUnder;
    }
    ...
    // Helper to verify a full merkle proof starting from some seedHash
    ...
    //Helper to check the placeBet receipt
